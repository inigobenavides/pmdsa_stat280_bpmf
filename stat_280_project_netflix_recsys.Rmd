...
for (i in 1:n_users) {
  index_sample <- sample(1:n_movies, size = sample(1:n_movies, 1))
...
map_df(
  1:n_users,
  function(user_index) {
    extract_simulated_ratings(simulation_results,
                              user_index = user_index, 
                              movie_index = 17) %>% 
      mutate(user_index = user_index)
  }
) %>% 
  ggplot() +
  geom_boxplot(
    mapping = aes(x = user_index,
                  y = simulated_rating,
                  group = user_index)
  ) +
  geom_label(
    data = function(data) {
      data %>% 
        group_by(user_index) %>% 
        summarise(median_rating = median(simulated_rating)) %>% 
        ungroup() %>% 
        mutate(num_ratings = sapply(
          user_index,
          function(x) {
            count_user_ratings(sparse_observations, x)
          }
        ))
    },
    mapping = aes(
      x = user_index, y = median_rating,
      label = num_ratings
    )
  )
