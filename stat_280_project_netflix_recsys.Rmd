...
source("code/svd.R") # order of sourcing is crucial
```
...
R_synthetic_tidy <- R_synthetic %>% matrix_to_tidydf()
...
sparse_observations <- R_synthetic_tidy %>% 
  sample_n(nrow(R_synthetic_tidy)/2)
sparse_observations %>% 
  mutate(value = sapply(value, transform_score_to_rating)) %>%
  vis_matrix(row_col = "row", column_col = "col",
             value_col = "value", label = TRUE) +
...
```{r}
# Visualisation for Ratings distribution
simulations.dt %>% 
  mutate(ratings = sapply(simulated_rating, transform_score_to_rating)) %>% 
  group_by(
    !!parse_expr('user_index'),
    !!parse_expr('movie_index'),
    !!parse_expr('ratings')
  ) %>% 
  summarise(count = n()) %>% 
  group_by(
  ...
  ungroup() %>% 
  # Add signal if included in sparse matrix
  full_join(
    sparse_observations,
    by = c('user_index' = 'row', 
           'movie_index' = 'col')
  ...
  ggplot() +
  geom_bar(
    mapping = aes(x = ratings, y = percentage,
                  fill = is_observed),
    stat = 'identity'
  ) +
  facet_grid(user_index ~ movie_index) +
  theme_bw() +
  # Add observed vline
  geom_vline(
    data = sparse_observations %>% 
      rename(user_index = row) %>% 
      rename(movie_index = col) %>% 
      mutate(value = sapply(value, transform_score_to_rating)),
  ...
  geom_vline(
    data = function(data) {
      data %>% group_by(user_index,
                        movie_index) %>% 
        summarise(
          mean = sum(ratings * count)/sum(count)
        )
    },
